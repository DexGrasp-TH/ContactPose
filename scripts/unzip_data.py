...
class ContactPoseDownloader(object):
    ...
    def _unzip_and_del(filename, dst_dir=None, progress=True, filter_fn=None):
        ...
        with ZipFile(filename) as f:
            ...
            members = None if filter_fn is None else list(filter(filter_fn, f.namelist()))
            f.extractall(dst_dir, members=members)
        os.remove(filename)

    def download_grasps(self):
        if os.path.exists(osp.join("data", "grasps")):
            pass
        else:
            filename = osp.join("data", "grasps.zip")
            print("Extracting...")
            self._unzip_and_del(filename, "data")
        p_ids = next(os.walk(os.path.join("data", "grasps")))[1]
        for p_id in tqdm(p_ids):
            if "full" not in p_id:
                continue
            sess_dir = osp.join("data/grasps", p_id)
    ...
    def download_depth_images(self, p_num, intent, dload_dir, include_objects=None):
        self.download_images(
            p_num,
            intent,
            dload_dir,
            include_objects,
            download_color=False,
            download_depth=True,
        )

    def download_color_images(self, p_num, intent, dload_dir, include_objects=None):
        self.download_images(
            p_num,
            intent,
            dload_dir,
    ...
    def download_images(
        ...
        assert osp.isdir(dload_dir), "Image download dir {:s} does not exist".format(dload_dir)
        p_id = "full{:d}_{:s}".format(p_num, intent)
                ...
                for cam_name in next(os.walk(images_dir))[1]:
                    for check_name in dirs_to_check:
                        ...
                        if is_nonempty_dir(check_dir):
                            print(
                                "{:s} {:s} already has extracted images, please delete {:s}".format(
                                    p_id, object_name, check_dir
                                )
                            )
                            ok = False
        if not ok:
        ...
        for object_name, filename in tqdm(filenames_to_extract.items()):
            ...
            for filename in next(os.walk(obj_dir))[-1]:
                if download_color and (not download_depth):
                    ...
                    if ".zip" not in filename:
                        continue
                    filter_fn = (lambda x: "color" not in x) if (not download_color) else None
                    self._unzip_and_del(osp.join(obj_dir, filename), progress=False, filter_fn=filter_fn)
