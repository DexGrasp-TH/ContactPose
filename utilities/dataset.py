...
def get_object_names(p_num, intent, ignore_hp=True):
    ...
    sess_dir = osp.join(
        osp.dirname(__file__), "..", "data", "contactpose_data", sess_dir
    )
...
class ContactPose(object):
    ...
    def __init__(self, p_num, intent, object_name, mano_pose_params=15, load_mano=True):
        """
        ...
        self.data_dir = osp.join(
            osp.dirname(__file__), "..", "data", "grasps", p_id, object_name
        )
        assert osp.isdir(self.data_dir)

        self.object_dir = osp.join(osp.dirname(__file__), "..", "data", "object_models")
        assert osp.isdir(self.object_dir)
        ...
        self._is_object_pose_optimized = [
            f["object_pose_optimized"] for f in ann["frames"]
        ]
        self._valid_hands = [
            hand_idx for hand_idx, hand in enumerate(ann["hands"]) if hand["valid"]
        ]
        ...
        self._im_filenames = [
            {k: v for k, v in zip(im_filenames.keys(), vv)}
            for vv in zip(*im_filenames.values())
        ]
        ...
        for hand_idx, hand in enumerate(ann["hands"]):
            if hand["valid"]:
                ...
                if hand["moving"]:
                    ...
                    oThs = [
                        np.linalg.inv(mutils.pose_matrix(f["hTo"][hand_idx]))
                        for f in ann["frames"]
                    ]
                    all_oTh.append(oThs)
        ...
        for frame_idx in range(len(self)):
            ...
            this_om = {}
            for camera_name in self.valid_cameras:
                this_om[camera_name] = mutils.project(
                    self.P(camera_name, frame_idx), oM
                )
                x = []
        ...
        if mutils.MANO_PRESENT and load_mano:
            ...
            print(
                "MANO code was not detected, please follow steps in README.md. "
    ...
    def __repr__(self):
        ...
        return (
            "Participant {:d}, intent {:s}, object {:s}\n".format(
                self.p_num, self.intent, self.object_name
            )
            + "{:d} frames\n".format(len(self))
    ...
    def object_filename(self):
        return osp.join(self.object_dir, "{:s}.ply".format(self.object_name))
    ...
    def mano_filename(self):
        ...
        """
        return osp.join(
            self.data_dir, "mano_fits_{:d}.json".format(self._mano_pose_params)
        )
    ...
    def mano_meshes(self, frame_idx=None):
        ...
        if frame_idx is None:
            frame_idx = np.random.choice(len(self))
        return mutils.load_mano_meshes(
            self.mano_params, ContactPose._mano_dicts, self._oTh[frame_idx]
        )
